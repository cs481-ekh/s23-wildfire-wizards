...
$CONTAINER cp ./sql/createtable.sql ffp-mysql:./var/lib/mysql-files/

$CONTAINER cp ./sql/$csv ffp-mysql:./var/lib/mysql-files/
...
waitcount=1
while ! $CONTAINER exec -i ffp-mysql mysql -uroot -pmysql-root-password -e "status" &> /dev/null ; do
...
$CONTAINER exec -i ffp-mysql mysql -uroot -pmysql-root-password  <<< "use ffp-mysql-db; source /var/lib/mysql-files/createtable.sql; LOAD DATA INFILE '/var/lib/mysql-files/${csv}' IGNORE INTO TABLE FPA_FOD_PLUS FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '${dqt}' LINES TERMINATED BY '\n' IGNORE 1 LINES SET CONT_DATE = date_format(str_to_date(CONT_DATE, '%m/%d/%Y %T'), '%Y-%m-%d %T');"
